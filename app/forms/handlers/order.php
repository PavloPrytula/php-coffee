...
function setProductsToOrder(int $orderId, array $cart): void
{
    ...
    foreach ($cart as $item) {
        ...
        minusProductQuantity($item['product_id'], $item['quantity']);

        if (!empty($item['additions'])) {
            $additions = $item['additions'];
            foreach ($item['additions'] as $addition) {
                minusProductQuantity($addition['product_id'], $addition['quantity']);
            }
        }
...
function minusProductQuantity(int $id, int $quantity): void
{
