...
function prepareCartItem(array $cart, array $dbProducts): array
{
    return array_map(
        function ($item) use ($dbProducts) {
            ...
            if (!empty($item['additions'])) {
                $item['additions'] = buildAdditionData(
                    $item['additions'],
                    $item['additions_qty'],
                    $dbProducts,
                    $item['product_id']
...
function buildAdditionData(array $additions, array $additionsQty, array $dbProducts, int $parentId)
{
    return array_map(
...
function mapCartsIds(array $cart): array
{
    $result = [];
...
function removeCartItem(array $fields)
{
    ...
    if (isset($fields['parent_key'])) {
        ...
        if (!empty($cart[$parent_key]['additions'])) {
            ...
            $cart[$parent_key]['additions_qty'] = array_values($cart[$parent_key]['additions']);
        } else {
